<?php 
    ...
    // 限制權限不等於1的不可進入此頁面
    if ($_SESSION["admin_user"]["data_type"] != 1) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        // 編輯
        if (isset($_POST['cname'], $_POST['id'], $_POST['data_type'])) {   // 檢查是否已提交要編輯的資料
            if (getUpdate($PDOLink, $_POST['cname'], $_POST['id'], $_POST['data_type'])) {
            ...
            // 創建帳號時判斷是否重複創建相同帳號
            $sql_check = "SELECT * FROM admin a WHERE a.id = ?";
