					...
					<div class="col-12">
						<!-- 付款 table -->
							...
							<?php 
								$j=0;						
								$ezcard_record_sql = "
									SELECT s.*, s.id as 'sid' FROM `seat` s WHERE 1 {$sql_kw} ORDER BY s.id LIMIT " . ($page-1) * $pagesize .", ".$pagesize;
								...
								while($row = $rs->Fetch()) {
									print " <tr>
												...
												<td>".$row['rate']."</td>
												<td>".$row['prepaid']."</td>
												<td>".get_refund($row['refundcertification'])."</td>
												<td width='10%' nowrap>
													<a onclick='set_refund(".$row['sid'].")' href='#'>退費設定</a>
												</td>
											</tr>";
								}
							?>
...
function search() {
	$('#mform').prop('action', 'RoomList2.php');
