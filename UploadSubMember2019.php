<?php
      ...
      if($col[A] && mb_strlen($col[A],"Big5") == strlen($col[A])){
          ...
          if($UserID){
              ...
              // if($AddRoomTotalRowNum < 7){
                ...
                $col="`room_number`";
                $col_data="'".$Room."' ";
                $ins_q="insert into member_upload (".$col.") values (".$col_data.") ";
                $PDOLink->exec($ins_q);
    ...
    // member_upload 房間初始化
    $RoomSetting_sql="select DISTINCT room_number from member_upload ";
    ...
    while($RoomSetting_sql_row=$RoomSetting_sql_rs->Fetch()){
		...
		$rs_room_number = $RoomSetting_sql_row[room_number];      
		$rs_RoomNumber =  $rs_room_number;
		$CreatedAtTime = date("Y-m-d H:i:s");
		$room_setting_RoomNumberTypes = substr($rs_RoomNumber,0,1);
    
		if($rs_room_number){

			/* 對硬體下命令初始化房間 */
			$roomKisokUpdateList_q="select center_id,meter_id from room where room_number='".$rs_room_number."'";
			...
			$col3="`title`,`computer_name`,`c_code`,`M0`,`M1`,`M2`,`M3`,`add_date`";
			$col_data3="'初始化','Web','".$InsertMemberUpdateC_code."','0','0','0','0',now()";
			$ins_q3="insert into system_setting (".$col3.") values (".$col_data3.") ";
			$PDOLink->exec($ins_q3);  
    ...
    /* 清空 member_upload */
    $SystemSettingChangeSql = "TRUNCATE TABLE member_upload";
    $SystemSettingChanges = $PDOLink->exec($SystemSettingChangeSql);   
